...
function generate_reform(us::BasicUncertaintySet, rm::Model, idxs::Vector{Int})
    ...
    for idx in idxs
        ...
        # this is for debugging only by zukui 2017.Dec.14
        # rmext = get_robust(rm)::RobustModelExt
        # con = rmext.unc_constraints[idx]
        # orig_lhs    = con.terms
        # for (uaff,var) in linearterms(orig_lhs)   # uaff  --> aᵢᵀu + bᵢ,    var --> xᵢ,    uaff.constant -->  bᵢ
        #     if string(var) == "ϕ_stochastic_program_obj"
        #         @step apply_reform(us, rm, idx)
        #     end
        # end

        apply_reform(us, rm, idx)  #@step zukui debugging start here																	
    end
end
...
function apply_reform(us::BasicUncertaintySet, rm::Model, idx::Int)
    ...
    # --------------------------------------------------------------------------
    # zukui added 2017.Dec.13, if the origial constraint is 0 <=/==/>= 0, do nothing
    orig_lhs = con.terms
    orig_rhs = JuMP.rhs(con)
    zeroLHSflag = true
    if !isempty(linearterms(orig_lhs))
        zeroLHSflag = false
    else
      for (coeff, uncparam) in linearterms(orig_lhs.constant)       #  coeff --> c, uncparam --> u
          if coeff != 0.0
            zeroLHSflag = false
            break
          end
    ...
    if zeroLHSflag == true && JuMP.rhs(con) == 0.0 # && JuMP.sense(con) == :(==)
      return 0 # this is 0=0 constraint
    end

    # --------------------------------------------------------------------------
    # zukui added 2017.Dec.14
    # If the problem to be solved with expectation objective
    # then do not derive robust counterpart of the objective function
    # use expectation of the objective function instead.
    # Caution: special name "ϕ_stochastic_program_obj" for the objective variable
    # is mandotory while you define the objective variable

    # test if current constraint defines the objective function
    objflag = false
    for (uaff,var) in linearterms(orig_lhs)   # uaff  --> aᵢᵀu + bᵢ,    var --> xᵢ,    uaff.constant -->  bᵢ
        if string(var) == "ϕ_stochastic_program_obj"
            objflag = true
    ...
    # the objective constraint  cᵀu + ∑ᵢ (aᵢᵀu + bᵢ) xᵢ <=> d
    # is converted to expectation counterpart using expectation of u
    if objflag == true

        new_lhs  = AffExpr()
        for (uaff,var) in linearterms(orig_lhs)   # uaff  --> aᵢᵀu + bᵢ,    var --> xᵢ,    uaff.constant -->  bᵢ
          sum_aiTu_b_i = uaff.constant  # get bᵢ
          for unc in 1:rmext.num_uncs
              for (coeff, uncparam) in linearterms(uaff)     # coeff --> aᵢ,  uncparam --> u
                if uncparam.id == unc
                  sum_aiTu_b_i = sum_aiTu_b_i + coeff * rmext.unc_expectation[unc]  # get ∑ᵢ aᵢᵀu
                end
              end
          end
          push!(new_lhs, sum_aiTu_b_i, var)
        end
        cTu = 0.0
        for unc in 1:rmext.num_uncs
          for (coeff, uncparam) in linearterms(orig_lhs.constant)     #  coeff --> c, uncparam --> u
              if uncparam.id == unc
                cTu = cTu + coeff * rmext.unc_expectation[unc]
              end
          end
        end
        new_lhs.constant = cTu

        @constraint(rm, new_lhs == orig_rhs)  # this constraint defines the expectation objective function
        
        return 0 # finish expectation processing, no further action on the objective
    end

    # --------------------------------------------------------------------------
    # zukui added 2017.Dec.13, to address uncertain equality constraint
    # if the original constraint is equality  cᵀu + ∑ᵢ (aᵢᵀu + bᵢ) xᵢ = d
    # then it is converted to following robust counterparts:
    # ∑ᵢ bᵢxᵢ = d     scalar constraint
    # cᵀ + ∑ᵢ xᵢaᵢᵀ = 0    vector constraints with size = size of uncertain parameter u
    if JuMP.sense(con) == :(==)
      orig_lhs    = con.terms
      orig_rhs    = JuMP.rhs(con)
      # add constraint ∑ᵢ bᵢxᵢ = d
      new_lhs     = AffExpr()
      for (uaff,var) in linearterms(orig_lhs)   # uaff  --> aᵢᵀu + bᵢ,    var --> xᵢ,    uaff.constant -->  bᵢ
          if uaff.constant != 0.0
              push!(new_lhs, uaff.constant, var)
          end
      end
      if !(new_lhs == zero(AffExpr) && orig_rhs == 0.0)  #if the constraint is 0 == 0, do nothing
        @constraint(rm, new_lhs == orig_rhs)
      end

      # then add vector constraints cᵀ + ∑ᵢ xᵢaᵢᵀ = 0
      for unc in 1:rmext.num_uncs
        new_lhs     = AffExpr()
        # get c
        for (coeff, uncparam) in linearterms(orig_lhs.constant)     #  coeff --> c, uncparam --> u
            if uncparam.id == unc
              new_lhs.constant = coeff
            end
        end
        # get ∑ᵢ xᵢaᵢᵀ
        for (uaff,var) in linearterms(orig_lhs)  # uaff  --> aᵢᵀu + bᵢ,    var --> xᵢ,
            for (coeff, uncparam) in linearterms(uaff)     # coeff --> aᵢ,  uncparam --> u
                if uncparam.id == unc
        ...
        if new_lhs != zero(AffExpr) #if the constraint is 0 == 0, do nothing
          @constraint(rm, new_lhs == 0.0)
        end
      end
      return 0 # end of equality constraint robust counterpart formulation
    end

    # --------------------------------------------------------------------------
    # comment (Zukui 2017.Dec.13): the following is the standard robust counterpart
    # derivation for inequality constraint based on duality
    # --------------------------------------------------------------------------
   ...
   # We also need the standalone cᵀu not related to any variable
        for (coeff, uncparam) in linearterms(orig_lhs.constant)     # orig_lhs.constant --> aᵀu,  coeff --> a, uncparam --> u
            rmext.unc_cat[uncparam.id] != :Cont &&
                error("Integer uncertain parameters not supported in reformulation.")

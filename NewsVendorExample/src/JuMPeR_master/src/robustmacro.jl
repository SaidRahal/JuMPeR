...
macro uncertain(args...)     # zukui comment: the condition in index of uncertain parameter is not implemented
#     @step test(m, args...)
# end
# function test(m, args...)
    ...
    expectation = Number[]   # zukui add 2017.Dec.14
    variance = Number[]    # zukui add 2017.Dec.16
    # for debugging said add 2019.Jul.31
    # println(kwargs)
    # println(extra)
    for ex in kwargs   # keyword arguments by original JuMPeR (only RO)
        kwarg = ex.args[1]
        println(kwarg)
        if kwarg == :basename
    ...
    for ex in extra   # keyword arguments in SP formulation; extrat in the context of JuMPeR RO
        kwarg = ex.args[1]
        if kwarg == :expectation   # zukui add 2017.Dec.14
            expectation = esc_nonconstant(ex.args[2])
        elseif kwarg == :variance   # zukui add 2017.Dec.16
            variance = esc_nonconstant(ex.args[2])
        else
            # MODIFICATION: error message
            error("in @uncertain ($var): Unrecognized keyword argument $kwarg")
        end
    ...
    if isa(var,Symbol)
        ...
        return assert_validmodel(m, quote
            ...
            #$(esc(var)) = Uncertain($m,$lb,$ub,$(quot(t)),string($quotvarname))   # zukui modify 2017.Dec.14
            $(esc(var)) = Uncertain($m,$lb,$ub,$(quot(t)),string($quotvarname), $expectation, $variance)
    ...
    # MODIFICATION: Variable to Uncertain, no start value
    #code = :( $(refcall) = Uncertain($m, $lb, $ub, $(quot(t)), JuMP.EMPTYSTRING) )     # zukui modify 2017.Dec.14
    code = :( $(refcall) = Uncertain($m, $lb, $ub, $(quot(t)), JuMP.EMPTYSTRING, $expectation, $variance) )
...
macro adaptive(args...)
    ...
    if isa(var,Symbol)
        ...
        return JuMP.assert_validmodel(m, quote
            ...
            #$(esc(var)) = Adaptive($m,$lb,$ub,$(quot(t)),string($quotvarname), $(quot(policy)), $(depends_on))
			$(esc(var)) = Adaptive($m,$lb,$ub,$(quot(t)),string($quotvarname),  $(quot(policy)), $(depends_on), string($symvarname) )   # zukui modify  2017.Dec.07  to save original variable name information (for the case that variable has no index)
            # MODIFICATION: No equivalent to registering variables
        end)
    end
    # MODIFICATION: error message
    ...
    # code = :( $(refcall) = Adaptive($m, $lb, $ub, $(quot(t)), JuMP.EMPTYSTRING, $(quot(policy)), $(depends_on) )   # original code, deactivated by Zukui


    # zukui modify  2017.Dec.07  to save original variable name information (for the case that variable has index)
    variablename = string(symvarname)

    if length(idxvars) == 1   # this variable has 1 index
      code = quote
          origname = $variablename * "["
          tmp = eval($(Expr(:escape, idxvars[1])))    #println(eval($(Expr(:escape, :i))))
          origname = origname * string(tmp)
          origname = origname * "]"
          $(refcall) = Adaptive($m, $lb, $ub, $(quot(t)), JuMP.EMPTYSTRING, $(quot(policy)), $(depends_on),  origname )
      end
    end

    if length(idxvars) == 2   # this variable has 2 index
      code = quote
          origname = $variablename * "["
          tmp = eval($(Expr(:escape, idxvars[1])))
          origname = origname * string(tmp)
          tmp = eval($(Expr(:escape, idxvars[2])))
          origname = origname * "," * string(tmp)
          origname = origname * "]"
          $(refcall) = Adaptive($m, $lb, $ub, $(quot(t)), JuMP.EMPTYSTRING, $(quot(policy)), $(depends_on),  origname )
      end
    ...
    if length(idxvars) == 3
      code = quote
          ...
          tmp = eval($(Expr(:escape, idxvars[2])))
          origname = origname * "," * string(tmp)
          tmp = eval($(Expr(:escape, idxvars[3])))
          origname = origname * "," * string(tmp)
          origname = origname * "]"
          $(refcall) = Adaptive($m, $lb, $ub, $(quot(t)), JuMP.EMPTYSTRING, $(quot(policy)), $(depends_on),  origname )
      end
    ...
    if length(idxvars) == 4
          # assuming that variable has 4 indices
          # use above similar method
